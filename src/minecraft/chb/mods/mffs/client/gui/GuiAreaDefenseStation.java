...
public class GuiAreaDefenseStation extends GuiContainer {
    ...
    protected void keyTyped(char c, int i) {
		...
		if (i != 1 && editMode) {
			...
			if(i ==14)
			NetworkHandlerClient.fireTileEntityEvent(DefenceStation, 12,"");
			
			if(i !=54 && i !=42 && i !=58 && i !=14)
			NetworkHandlerClient.fireTileEntityEvent(DefenceStation, 11,String.valueOf(c));
	...
	protected void mouseClicked(int i, int j, int k) {
		...
		if (editMode){
			...
			NetworkHandlerClient.fireTileEntityEvent(DefenceStation, 10,"null");
			editMode = true;
		}
	}
...
public void initGui() {
	...
	controlList.add(new GraphicButton(101, (width / 2) - 36, (height / 2) - 58,DefenceStation,2));
	controlList.add(new GraphicButton(102, (width / 2) +20, (height / 2) - 58,DefenceStation,3));
    super.initGui();
}
...
@Override
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		fontRenderer.drawString(DefenceStation.getDeviceName(), 120, 9, 0x404040);
