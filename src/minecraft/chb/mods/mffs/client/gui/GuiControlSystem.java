...
public class GuiControlSystem extends GuiContainer {
    ...
    protected void keyTyped(char c, int i) {
		...
		if (i != 1 && editMode) {
			...
			NetworkHandlerClient.fireTileEntityEvent(ControlSystem, 11,String.valueOf(c));
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
    	int textur = mc.renderEngine.getTexture("/chb/mods/mffs/sprites/GuiControlSystem.png");
	...
	protected void actionPerformed(GuiButton guibutton) {
		...
		NetworkHandlerClient.fireTileEntityEvent(ControlSystem,guibutton.id,"");
		}
	}
	...
	public void initGui() {
		...
		controlList.add(new GraphicButton(101, (width / 2) -115, (height / 2) - 25,ControlSystem,2));
		controlList.add(new GraphicButton(102, (width / 2) -115, (height / 2) - 5,ControlSystem,3));
		controlList.add(new GuiButton(103, (width / 2) + -65, (height / 2) -8,100,20,"Open Remote Gui"));
		super.initGui();
	}
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		fontRenderer.drawString(ControlSystem.getDeviceName(), 123, 9, 0x404040);
		
		fontRenderer.drawString("DataLink", 190, 54, 0x404040);
		fontRenderer.drawString("Reader", 190, 65, 0x404040);
    
		
		fontRenderer.drawString("Name: "+ControlSystem.getRemoteDeviceName(), 15, 30, 0x404040);
		fontRenderer.drawString("Type:  "+ControlSystem.getRemoteDeviceTyp(), 15, 45, 0x404040);
		if(ControlSystem.getStackInSlot(1)!=null){
		RenderItem renderItem = new RenderItem();
		RenderHelper.enableGUIStandardItemLighting();
		renderItem.renderItemIntoGUI(mc.fontRenderer, mc.renderEngine, new ItemStack(ModularForceFieldSystem.MFFSItemSecLinkCard), 40, 59);
		renderItem.renderItemIntoGUI(mc.fontRenderer, mc.renderEngine, new ItemStack(ModularForceFieldSystem.MFFSitemfc), 100, 59);
		RenderHelper.disableStandardItemLighting();
		if(ControlSystem.getRemoteSecurityStationlink()){
		fontRenderer.drawString("linked", 60, 64, 0x228B22);}else{
		...
		fontRenderer.drawString("Power left: "+ControlSystem.getRemotePowerleft()+"%", 40, 80, 0x404040);
		}else{
		fontRenderer.drawString("linked", 120, 64, 0x8B1A1A);}
		
		if(ControlSystem.getRemoteGUIinRange()){
		fontRenderer.drawString("OK", 40, 107, 0x228B22);}else{
