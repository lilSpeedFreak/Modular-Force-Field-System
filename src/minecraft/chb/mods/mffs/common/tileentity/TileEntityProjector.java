...
public class TileEntityProjector extends TileEntityFEPoweredMachine implements IModularProjector{
	...
	public void checkslots() {
		...
		if(hasOption(ModularForceFieldSystem.MFFSProjectorOptionFieldFusion,true)){
			if(!Linkgrid.getWorldMap(worldObj).getFieldFusion().containsKey(getDeviceID()))
				Linkgrid.getWorldMap(worldObj).getFieldFusion().put(getDeviceID(), this);
			}else{
			if(Linkgrid.getWorldMap(worldObj).getFieldFusion().containsKey(getDeviceID()))
				Linkgrid.getWorldMap(worldObj).getFieldFusion().remove(getDeviceID());
		 }
		
		if(hasOption(ModularForceFieldSystem.MFFSProjectorOptionForceFieldJammer,false)){
			if(!Linkgrid.getWorldMap(worldObj).getJammer().containsKey(getDeviceID()))
				Linkgrid.getWorldMap(worldObj).getJammer().put(getDeviceID(), this);
			}else{
			if(Linkgrid.getWorldMap(worldObj).getJammer().containsKey(getDeviceID()))
				Linkgrid.getWorldMap(worldObj).getJammer().remove(getDeviceID());
		 }
		...
		if (hasValidTypeMod()) {
			...
			for (int spot = 2; spot <= 4; spot++) {
			 if(getStackInSlot(spot) != null){
			  ...
			  if(getStackInSlot(spot) != null){
			  if(getStackInSlot(spot).getItem() instanceof ItemProjectorOptionForceFieldJammer && isPowersourceItem()){
				dropplugins(spot,this);
			  }
			  }
			  
			  if(getStackInSlot(spot) != null){
			  if(getStackInSlot(spot).getItem() instanceof ItemProjectorOptionFieldFusion && isPowersourceItem()){
				dropplugins(spot,this);
			  }
			  }
			  
			  if(getStackInSlot(spot) != null){
			  if(getStackInSlot(spot).getItem() instanceof ItemProjectorOptionDefenseStation && isPowersourceItem()){
				dropplugins(spot,this);
			  }
			  }
			  ...
			  if(getStackInSlot(12).getItem() instanceof ItemCardSecurityLink && isPowersourceItem()){
				dropplugins(12,this);
			  }
			  }
			
			if(!this.hasOption(ModularForceFieldSystem.MFFSProjectorOptionCamouflage,true))
				dropplugins(11,this);
	...
	private void UpdateForcefieldTexttur() {
		if(this.isActive() && this.hasOption(ModularForceFieldSystem.MFFSProjectorOptionCamouflage,true))
		{
	...
	public void updateEntity() {
		if (worldObj.isRemote == false) {
			...
			if ((getSwitchValue() && (switchdelay >= 40))
			...
			if ((!getSwitchValue() && switchdelay >= 40)
					|| !hasValidTypeMod() || !hasPowerSource() || burnout
	...
	public void FieldGenerate(boolean init) {
		...
		for (PointXYZ pnt : field_def) {
			...
			ForceFieldBlockStack ffb = WorldMap.getForceFieldWorld(worldObj).getForceFieldStackMap(pnt.hashCode());

			if(ffb!=null){
				
				if(ffb.isSync())continue;
	...
	public boolean isItemValid(ItemStack par1ItemStack, int Slot) {
		...
		if(Slot == 11 && par1ItemStack.itemID < 4096  && this.hasOption(ModularForceFieldSystem.MFFSProjectorOptionCamouflage,true))return true;
