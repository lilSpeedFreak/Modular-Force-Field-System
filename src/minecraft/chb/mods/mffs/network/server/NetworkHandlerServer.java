...
public void onPacketData(INetworkManager manager,Packet250CustomPayload packet, Player player) {
	...
	case 10:
		...
		for(String varname : Corrdinsaten.split("#"))
		{
		 ...
		 {
				ForceFieldBlockStack ffworldmap = WorldMap.getForceFieldWorld(worldserver).getForceFieldStackMap(new PointXYZ(servertileEntity.xCoord,servertileEntity.yCoord,servertileEntity.zCoord,worldserver).hashCode());
...
public static void updateTileEntityField(TileEntity tileEntity, String varname)
{
 ...
 {
	 try {	
        Field f = ReflectionHelper.findField(TileEntitySecStorage.class, varname);
        f.get(tileEntity);
    	dos.writeUTF(String.valueOf(f.get(tileEntity)));
	} catch (Exception e) {
		if(DEBUG)
 ...
 {
	 try {	
        Field f = ReflectionHelper.findField(TileEntityControlSystem.class, varname);
        f.get(tileEntity);
    	dos.writeUTF(String.valueOf(f.get(tileEntity)));
	} catch (Exception e) {
		if(DEBUG)
